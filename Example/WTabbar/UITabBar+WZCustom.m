...
@implementation UITabBar (WZCustom)

//+ (void)load
//{
//    static dispatch_once_t onceToken;
//    dispatch_once(&onceToken, ^{
        /// hook SelectedItem的set方法
//        Method setSelectedItem_originalMethod = class_getInstanceMethod([self class], @selector(setSelectedItem:));
//        Method setSelectedItem_swizzledMethod = class_getInstanceMethod([self class], @selector(wz_setSelectedItem:));
//        method_exchangeImplementations(setSelectedItem_originalMethod, setSelectedItem_swizzledMethod);
        
        /// 更新布局
//        Method layoutSubviews_originalMethod = class_getInstanceMethod([self class], @selector(layoutSubviews));
//        Method layoutSubviews_swizzledMethod = class_getInstanceMethod([self class], @selector(wz_layoutSubviews));
//        method_exchangeImplementations(layoutSubviews_originalMethod, layoutSubviews_swizzledMethod);
        
//        Method hitTest_originalMethod = class_getInstanceMethod([self class], @selector(hitTest:withEvent:));
//        Method hitTest_swizzledMethod = class_getInstanceMethod([self class], @selector(wz_hitTest:withEvent:));
//        method_exchangeImplementations(hitTest_originalMethod, hitTest_swizzledMethod);
//    });
//}

/// 点击边缘
- (UIView *)wz_hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    [self wz_hitTest:point withEvent:event];
...
/// 会执行2次需要过滤一下
- (void)wz_setSelectedItem:(UITabBarItem *)item{
    [self wz_setSelectedItem:item];
    ...
    for (UITabBarItem *itemA in self.items) {
        itemA.wzIsSelect = item != itemA ? NO:YES;
        if (itemA.wzCustomView) {
...
/// 布局操作
- (void)wz_layoutSubviews{
    [self wz_layoutSubviews];
